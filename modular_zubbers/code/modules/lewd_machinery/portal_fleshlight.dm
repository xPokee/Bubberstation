...
/obj/item/clothing/sextoy/portal_fleshlight/proc/updatesleeve()
	...
	if(target_wearer) // If the portal panties are worn
		...
		if(linked_panties.current_target == ORGAN_SLOT_VAGINA || linked_panties.current_target == ORGAN_SLOT_ANUS || linked_panties.current_target == BODY_ZONE_PRECISE_MOUTH)
			sleeve = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_sleeve_normal")

			if(islizard(target_wearer))
				sleeve = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_sleeve_lizard")
			else if(isunathi(target_wearer))
				sleeve = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_sleeve_lizard")
			else if(isakula(target_wearer))
				sleeve = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_sleeve_akula")
			else if(isslimeperson(target_wearer))
				sleeve = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_sleeve_slime")
			else if(ismammal(target_wearer))
				sleeve = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_sleeve_fluff")
		...
		switch(linked_panties.current_target)
			if(ORGAN_SLOT_VAGINA)
				...
				organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_vag")
				switch(vag.genital_name)
					if("Human")
						organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_vag")
					if("Puffy")
						organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_vag_puffy")
					if("Gaping")
						organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_vag_gaping")
					if("Spade")
						organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_vag_spade")
					if("Feline")
						organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_vag_feline")
					if("Equine")
						organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_vag_equine")
					if("Cervine")
						organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_vag_cervine")
					if("Sergal")
						organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_vag_sergal")
					if("Cloaca")
						organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_vag_cloacal")
					if("Hemi")
						organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_vag_hemi")
				if(vag.uses_skin_color)
					organ.color = target_wearer.dna.features["mcolor"]
				else
					organ.color = vag.color
				if(vag.aroused == AROUSAL_FULL)
					add_overlay(mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_vag_drip"))
			if(ORGAN_SLOT_ANUS)
				organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_anus")
				organ.color = target_organ.color
			if(ORGAN_SLOT_PENIS)
				var/obj/item/organ/genital/penis/penis = target_organ
				organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "penis")
				switch(penis.genital_name)
					if("Human")
						organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "penis")
					if("Knotted", "Barbed, Knotted")
						organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "knotted")
					if("Flared")
						organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "flared")
					if("Tapered")
						organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "tapered")
					if("Tentacled")
						organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "tentacle")
					if("Hemi", "Knotted Hemi")
						organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "hemi")
				if(penis.uses_skin_color)
					organ.color = target_wearer.dna.features["mcolor"]
				else
					organ.color = penis.color
				if(penis.aroused == AROUSAL_FULL && penis.sheath != SHEATH_NONE)
					add_overlay(mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_penis_drip"))
			if(BODY_ZONE_PRECISE_MOUTH)
				add_overlay(mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_mouth"))
				organ = mutable_appearance('modular_zubbers/icons/obj/lewd.dmi', "portal_mouth_lips")
