...
/mob/living/
	var/list/status_indicators = list()
#define STATUS_INDICATOR(X) /datum/status_indicator/##X;\
/datum/status_indicator/##X/Initialize(){\
	icon_state = ##X;\
	show_indicator(##X);\
}\
/datum/status_indicator/##X

#define STATUS_INDICATOR_TYPE(X) /datum/status_indicator/##X
#define ARG2INDICATOR(X) /datum/status_indicator/(text2path(#X))
#define ARG2PATH(X, Y) text2path(#X#Y)
/datum/status_indicator/
	var/icon = 'modular_zubbers/icons/mob/status_indicators.dmi'
	var/icon_state
	var/image/held_indicator
	var/added


STATUS_INDICATOR_TYPE(stunned)
	icon_state = STUNNED
STATUS_INDICATOR_TYPE(weaken)
	icon_state = WEAKEN
STATUS_INDICATOR_TYPE(paralysis)
	icon_state = PARALYSIS
STATUS_INDICATOR_TYPE(sleeping)
	icon_state = SLEEPING
STATUS_INDICATOR_TYPE(confused)
	icon_state = CONFUSED

/datum/status_indicator/proc/show_indicator(new_indicator, icon_scale)
	if(!added)
		new_indicator = new/image(icon, icon_state)
		held_indicator = new_indicator
	. = held_indicator

/datum/status_indicator/proc/del_indicator(new_indicator)
	held_indicator = null // hard del?
	QDEL_NULL(src)

/datum/status_indicator/proc/handle_self_adding(mob/living/current_mob)
	if(!added)
		current_mob.add_overlay(held_indicator)
		added = TRUE

/datum/component/status_indicator/ // Logic handler for adding and removing
...
/datum/component/status_indicator/proc/handle_status_indicators()
	...
	var/expected_status_indicator_length = (STATUS_INDICATOR_ICON_X_SIZE * length(attached_mob.status_indicators) + (STATUS_INDICATOR_ICON_MARGIN * max(length(attached_mob.status_indicators) - 1, 0)))
	...
	for(var/datum/status_indicator/current in attached_mob.status_indicators)
		var/image/indicator = current.held_indicator
		...
		current.handle_self_adding(attached_mob)
		...
		message_admins("handle check at [current.type]")

/datum/component/status_indicator/proc/add_status_indicator(prospective_indicator, index)
	if(prospective_indicator && !attached_mob.status_indicators[prospective_indicator])
		var/path = text2path("/datum/status_indicator/[prospective_indicator]")
		message_admins(path)
		if(ispath(path, /datum/status_indicator))
			attached_mob.status_indicators |= new path
			message_admins("created a [path]")
			handle_status_indicators()
/datum/component/status_indicator/proc/remove_status_indicator(prospective_indicator, index) // This will be spammed
	if(prospective_indicator)
		var/datum/status_indicator/current = attached_mob.status_indicators[prospective_indicator]
		if(current)
			current.del_indicator()
			handle_status_indicators()
/// Cuts all the indicators on a mob in a loop.
/datum/component/status_indicator/proc/cut_indicators_overlays()
	SIGNAL_HANDLER
	for(var/datum/status_indicator/current in attached_mob.status_indicators)
		attached_mob.cut_overlay(current)

/datum/component/status_indicator/proc/weaken_indicator_update()
	SIGNAL_HANDLER
	is_weakened() ? add_status_indicator(WEAKEN, 1) : remove_status_indicator(WEAKEN, 1)
/datum/component/status_indicator/proc/paralyzed_indicator_update()
	SIGNAL_HANDLER
	is_paralyzed() ? add_status_indicator(PARALYSIS, 2) : remove_status_indicator(PARALYSIS, 2)
/datum/component/status_indicator/proc/stunned_indicator_update()
	SIGNAL_HANDLER
	is_stunned() ? add_status_indicator(STUNNED, 3) : remove_status_indicator(STUNNED, 3)
/datum/component/status_indicator/proc/unconcious_indicator_update()
	SIGNAL_HANDLER
	is_unconcious() ? add_status_indicator(SLEEPING, 4) : remove_status_indicator(SLEEPING, 4)
/datum/component/status_indicator/proc/confused_indicator_update()
	SIGNAL_HANDLER
	is_confused() ? add_status_indicator(CONFUSED, 5) : remove_status_indicator(CONFUSED, 5)
...
/datum/component/status_indicator/proc/apply_pref_on_login() // shitcode pls help
